...
- [x] **Phase 5 — A vs D ablation** (see [`results/phase4/PHASE5_CORRECTED.md`](results/phase4/PHASE5_CORRECTED.md)). The first "8σ A beats D negative result" was a **M-RoPE bug** in D's `inputs_embeds` injection (found via `scripts/p5_debug_stage2.py`). Fixed with **D-hook** (additive conditioning, full `input_ids` path). Corrected result: D-hook **ties A** on tap/swipe (action type uninformative) and **significantly beats A on `all_with_coords`** (action type informative via `type`→fields). **Paired bootstrap** (750 paired units): hit@0.10 +0.045 (95% CI [+0.017,+0.073], p=0.002), hit@0.25 +0.055 (p<0.001), mean L2 −0.027 (p<0.001) — all 4 metrics significant.
...
- [ ] **Writeup + stretch** (final report, poster; optional: scale n, conservative example-level bootstrap for the taps/swipes control, 7B run).

Cumulative Modal spend: **~$45 of $200 (23%).**
