...
- [x] **Phase 5 — A vs D ablation** (see [`results/phase4/PHASE5_CORRECTED.md`](results/phase4/PHASE5_CORRECTED.md)). The first "8σ A beats D negative result" was a **M-RoPE bug** in D's `inputs_embeds` injection (found via `scripts/p5_debug_stage2.py`). Fixed with **D-hook** (additive conditioning, full `input_ids` path). Validated result matches the hypothesis: D-hook **ties A** on tap/swipe (action type uninformative) and **significantly beats A on `all_with_coords`** (action type informative via `type`→fields). **Paired bootstrap** (750 paired units): hit@0.10 +0.045 (95% CI [+0.017,+0.073], p=0.002), hit@0.25 +0.055 (p<0.001), mean L2 −0.027 (p<0.001) — all 4 metrics significant.
...
- [x] **Phase 6 — full A/B/C/D ablation** ([`results/phase4/PHASE6_ABLATION_ABCD.md`](results/phase4/PHASE6_ABLATION_ABCD.md)). all_with_coords, paired bootstrap: **B ≈ D > C > A.** Both auxiliary loss (B) and our embedding (D) significantly beat flat (A) on all metrics (p<0.002); B and D are statistically tied (mechanism doesn't matter over the training signal); hard routing (C) is weakest. Lands on the plan's "beat A but tie B" rubric outcome.
...
- [ ] **Writeup + stretch** (final report, poster; optional: B/C on taps_and_swipes control, scale n, attention viz, 7B run).

Cumulative Modal spend: **~$31 of $200 (16%).**
