...
- [~] **Phase 5 — A vs D ablation** (see [`results/phase4/PHASE5_CORRECTED.md`](results/phase4/PHASE5_CORRECTED.md) and [`SESSION_LOG.md`](SESSION_LOG.md)). Plot twist: the first "8σ A beats D negative result" was a **M-RoPE bug** in D's `inputs_embeds` injection (found via `scripts/p5_debug_stage2.py`). Fixed with **D-hook** (additive conditioning, full `input_ids` path). Corrected result matches the hypothesis: D-hook **ties A** on tap/swipe (action type uninformative) and **beats A on all 4 metrics on `all_with_coords`** (action type informative via `type`→fields; hit@0.25 +0.045/2.2σ, mean L2 −0.024/3.1σ). One open cell: A seed 46 (cancelled twice) → finish the 5×5 + paired bootstrap next session.
...
- [ ] **Phase 6 — stretch + writeup** (paired bootstrap, scale n, variants B/C, attention viz, final report, poster).

Cumulative Modal spend: **~$22 of $200 (11%).**
