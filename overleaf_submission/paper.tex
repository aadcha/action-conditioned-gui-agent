...
\end{center}
\caption{Qualitative grounding predictions on held-out AITW screenshots: ground truth (green $\circ$), flat baseline A (red $\times$), and conditioned D-hook (blue $\times$); each panel reports both per-example normalized distances. Conditioning rescues \emph{click} targets the flat baseline misses (left), both models succeed on easy clicks, and both fail on \emph{type} actions whose AITW touch coordinates are degenerate (corner-pinned), visually confirming the per-class analysis in Table~\ref{tab:perclass}.}
...
% ---- OPTIONAL: attention-visualization figure (asset already in figures/) ----
% Uncomment this block to include the attention heatmap. NOTE: if you use it,
% update the "Future directions" sentence in the Conclusion that currently lists
% attention visualization as future work, since you would then have done it.
% \begin{figure}[t]
% \begin{center}
% \includegraphics[width=0.98\linewidth]{figures/attn_example_1.png}
% \end{center}
% \caption{Image-region attention for a held-out \emph{click} example under three
% action-type conditionings. Conditioning on the correct type (click) places more
% attention mass on the screenshot ($0.204$) than conditioning on \emph{type}
% ($0.132$) or \emph{scroll} ($0.110$), indicating the conditioning signal reshapes
% spatial attention.}
% \label{fig:attention}
% \end{figure}
...
\textbf{Supported}: Factoring action-type prediction from spatial grounding significantly improves hit rates when action type is spatially informative (click-vs-scroll/type discrimination). The advantage is largest at low data and concentrated in the click class. A full two-stage pipeline with predicted types beats the flat end-to-end baseline end-to-end.
...
\textbf{Mechanistic insight}: The grounding gain is click-specific click disambiguation (68\% of val), not field localization. AITW \texttt{type} coordinates are degenerate, so the residual vision-grounding challenge on this dataset is click-vs-scroll/type classification, not precise text-field targeting.

\textbf{Future directions}: The auxiliary-loss finding suggests that better action-type training signal — even without explicit inference-time routing — is the more promising lever. Future work could explore attention visualization to confirm that the auxiliary loss reshapes spatial attention differently for click vs.\ scroll, test the conditioning advantage on ScreenSpot~\cite{seeclick} where grounding targets are diverse, and extend to larger backbones (Qwen2-VL-7B) to test whether the scaling behavior changes.
