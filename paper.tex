...
\paragraph{Overfitting and stability.} Several design choices limit overfitting. Stage~1 trains only a small MLP on features from a \emph{frozen} backbone and selects the checkpoint by validation macro-F1 (early stopping). Stage~2 updates only the LoRA adapters and the action embedding, roughly 0.2\% of parameters (4.4M of 2.2B), for two epochs, so the model has limited capacity to memorize. The principal concern is therefore variance rather than overfitting: on the small Stage~2 validation sets ($n \approx 250$), hit@$r$ can swing 15 to 20 points between the two epochs. We therefore report the mean $\pm$ standard deviation across three seeds as our uncertainty estimate rather than a single-epoch number, and all headline comparisons use a paired bootstrap over pooled per-example distances.
...
\midrule
gold $-$ wrong & $+0.192^{**}$ & $+0.329^{**}$ & n/a \\
gold $-$ zero & $+0.093^{**}$ & $+0.112^{**}$ & n/a \\
\bottomrule
\end{tabular}
\end{center}
